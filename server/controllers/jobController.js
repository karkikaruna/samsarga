...
export const postJob = catchAsyncErrors(async (req, res, next) => {
  ...
  if (salaryFrom && salaryTo && fixedSalary) {
    return next(new ErrorHandler("Please select either fixed salary or ranged salary, not both.", 400));
  }

  const postedBy = req.user._id;
  const job = await Job.create({
    title, description, category, country,
    city, location, fixedSalary, salaryFrom, salaryTo, postedBy,
  });
...
export const getSingleJob = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const job = await Job.findById(id);
    if (!job) {  // express-fileupload can return array or single object — normalise
      return next(new ErrorHandler("Job not found.", 404));
    }
    res.status(200).json({
      success: true,
